...
const multer = require('multer');//nueva linea
const path = require('path');//nueva linea
const fs = require('fs');//nueva 
...
rutas.post('/', upload.single('imagen'),async (req, res) => {
...
rutas.put('/:id',upload.single('imagen'), async (req, res) => {
    const { producto, categoria, existencia, precio } = req.body;
    const imagen = req.file ? req.file.filename : null;
        ...
        if (product.imagen) {
            ...
            fs.unlink(imagePath, (err) => {
                if (err) {
                    ...
                    console.log('Imagen eliminada:', product.imagen);
                }
            })
        }
